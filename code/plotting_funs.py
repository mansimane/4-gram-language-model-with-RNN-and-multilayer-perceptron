...
def plot_loss_train_valid (train_p, val_p,train_ce, valid_ce ,  hyper_para):
    ...
    plt.legend(['Train Cross Entropy Loss', 'Valid Cross Entropy Loss'], loc='upper right')
    ...
    plt.suptitle(a + b + c + d + f)

    fig = plt.gcf()
    fig.savefig('../results/'+a+b+c+d+e+f+'/Q3_2_error' + date + '.png')
    #plt.show()
    ...
    plt.suptitle(a + b + c + d + e + f)
    plt.legend(['Train Perplexity', 'Val Perplexity'], loc='upper right')
    fig = plt.gcf()
    fig.savefig('../results/'+a+b+c+d+e+f+'/Q3_2_per' + date + '.png')
    # plt.show()
    # plt.savefig()
    plt.close()
